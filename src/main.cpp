...
void draw_pipes(float offset_x, float d, float y)
{
    if (d - get_distance() - 13 < 144)
    {
        vita2d_draw_texture_part_scale(
            sprites, offset_x - 26 + 2 * (d - get_distance()), y * 2 - 354, 514, 0, 26, 160, 2, 2);
        vita2d_draw_texture_part_scale(
            sprites, offset_x - 26 + 2 * (d - get_distance()), 66 + y * 2, 542, 0, 26, 160, 2, 2);
    }
}
...
void draw_game()
{
    ...
    if (sprite == 3 || BIRD.velocity > 1.5f)
        sprite = 1;
    sprite += BIRD.color + BIRD.color + BIRD.color;
    vita2d_draw_texture_part_scale_rotate(
        sprites, BIRD.position.x * 2 + 336, 16 + BIRD.position.y + BIRD.position.y, 295 + 28 * sprite, 242, 19, 14, 2,
        2, BIRD.rot);
    vita2d_draw_texture_part_scale(
        sprites, 336 - mod(get_distance() + get_distance(), 288), 416, 292, 0, 144, 56, 2, 2);
    vita2d_draw_texture_part_scale(
        sprites, 624 - mod(get_distance() + get_distance(), 288), 416, 292, 0, 144, 56, 2, 2);
    if (get_state() != state_menu && get_time() - death_timer < 1000)
        draw_score(336);
    static int n = 0;
    if (get_time() - death_timer > 1000)
    {
        ...
        vita2d_draw_texture_tint_part_scale(
            sprites, 336 + 144 - 96, 16 + 256 - 21 - 110 - 15 * sin(temp * 3.141592f), 395, 59, 96, 21, 2, 2,
            RGBA8(255, 255, 255, (int)(255 * temp)));
        if (get_time() - death_timer > 1600)
        {
            float temp = 1 - ((get_time() - death_timer) > 2200 ? 1 : (get_time() - death_timer - 1600) / 600.0f);
            vita2d_draw_texture_part_scale(
                sprites, 336 + 144 - 113, 16 + 256 - 57 + (256 + 57) * (temp * temp * temp), 397, 180, 113, 57, 2, 2);
            int score = 0;
            if (get_time() - death_timer > 2200)
            {
                ...
                {
                    vita2d_draw_texture_tint_part_scale(
                        sprites, 336 + 144 - 52, 16 + 256 - 29 + 110, 398, 128, 52, 29, 2, 2,
                        RGBA8(255 - 100 * butt_flag, 255 - 100 * butt_flag, 255 - 100 * butt_flag, 255));
                }
            }
            int medal = -1;
            if (get_score() >= 10)
            ...
            {
                vita2d_draw_texture_part_scale(
                    sprites, 336 + 144 + 113 - 20 - 180, 16 + 256 - 57 + (256 + 57) * (temp * temp * temp) + 32 + 10,
                    291 + 23 * medal, 198, 22, 22, 2, 2);
                ...
                {
                    vita2d_draw_texture_part_scale(
                        sprites, 336 + 144 + 113 - 20 - 180 + 20 + blink.x,
                        16 + 256 - 57 + (256 + 57) * (temp * temp * temp) + 32 + 10 + 20 + blink.y, 384,
                        94 + sprite * 26, 5, 5, 2, 2);
                }
            }
            draw_score_small(336, 144 + 113 - 20, 16 + 256 - 57 + (256 + 57) * (temp * temp * temp) + 32, score);
            draw_score_small(
                336, 144 + 113 - 20, 16 + 256 - 57 + (256 + 57) * (temp * temp * temp) + 32 + 42,
                score < get_record() ? get_record() : score);
            if (score > get_record())
                vita2d_draw_texture_part_scale(
                    sprites, 336 + 144 + 113 - 90, 16 + 256 - 57 + 32 + 26, 365, 148, 16, 7, 2, 2);
        }
    }
    else
    {
    ...
    vita2d_draw_texture_tint_part_scale(
        sprites, 336 + 144 - 92, 16 + 256 - 25 - 90, 295, 59, 92, 25, 2, 2,
        RGBA8(255, 255, 255, (int)(255 * tap_fade * tap_fade)));
    vita2d_draw_texture_tint_part_scale(
        sprites, 336 + 144 - 57, 16 + 256 - 33, 292, 91, 57, 49, 2, 2,
        RGBA8(255, 255, 255, (int)(255 * tap_fade * tap_fade)));
    if (get_state() == state_menu)
    {
        vita2d_draw_texture_part_scale(sprites, 336 + 144 - 89, 16 + 256 - 25 - 120, 408, 96, 89, 25, 2, 2);
        vita2d_draw_texture_tint_part_scale(
            sprites, 336 + 144 - 52, 16 + 256 - 29 + 10, 398, 128, 52, 29, 2, 2,
            RGBA8(255 - 100 * butt_flag, 255 - 100 * butt_flag, 255 - 100 * butt_flag, 255));
    }
    if (fade > 0)
...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        butt_flag |=
            (get_state() == state_menu && touch.reportNum > 0 && touch.report[0].x / 2 > 336 + 144 - 52 &&
             touch.report[0].y / 2 > 16 + 256 - 29 + 10 && touch.report[0].x / 2 < 336 + 144 + 52 &&
             touch.report[0].y / 2 < 16 + 256 + 29 + 10);
        butt_flag |=
            (get_state() == state_death && touch.reportNum > 0 && touch.report[0].x / 2 > 336 + 144 - 52 &&
             touch.report[0].y / 2 > 16 + 256 - 29 + 110 && touch.report[0].x / 2 < 336 + 144 + 52 &&
             touch.report[0].y / 2 < 16 + 256 + 29 + 110);
        butt_flag |=
            (get_state() == state_menu && touch.reportNum > 1 && touch.report[1].x / 2 > 336 + 144 - 52 &&
             touch.report[1].y / 2 > 16 + 256 - 29 + 10 && touch.report[1].x / 2 < 336 + 144 + 52 &&
             touch.report[1].y / 2 < 16 + 256 + 29 + 10);
        butt_flag |=
            (get_state() == state_death && touch.reportNum > 1 && touch.report[1].x / 2 > 336 + 144 - 52 &&
             touch.report[1].y / 2 > 16 + 256 - 29 + 110 && touch.report[1].x / 2 < 336 + 144 + 52 &&
             touch.report[1].y / 2 < 16 + 256 + 29 + 110);
        bool pressed = !(pad.buttons & SCE_CTRL_CROSS) && oldpad.buttons & SCE_CTRL_CROSS;
        pressed |=
            (get_state() == state_menu && touch.reportNum < 1 && oldtouch.reportNum == 1 &&
             oldtouch.report[0].x / 2 > 336 + 144 - 52 && oldtouch.report[0].y / 2 > 16 + 256 - 29 + 10 &&
             oldtouch.report[0].x / 2 < 336 + 144 + 52 && oldtouch.report[0].y / 2 < 16 + 256 + 29 + 10);
        pressed |=
            (get_state() == state_death && touch.reportNum < 1 && oldtouch.reportNum == 1 &&
             oldtouch.report[0].x / 2 > 336 + 144 - 52 && oldtouch.report[0].y / 2 > 16 + 256 - 29 + 110 &&
             oldtouch.report[0].x / 2 < 336 + 144 + 52 && oldtouch.report[0].y / 2 < 16 + 256 + 29 + 110);
        pressed |=
            (get_state() == state_menu && touch.reportNum < 2 && oldtouch.reportNum == 2 &&
             oldtouch.report[1].x / 2 > 336 + 144 - 52 && oldtouch.report[1].y / 2 > 16 + 256 - 29 + 10 &&
             oldtouch.report[1].x / 2 < 336 + 144 + 52 && oldtouch.report[1].y / 2 < 16 + 256 + 29 + 10);
        pressed |=
            (get_state() == state_death && touch.reportNum < 2 && oldtouch.reportNum == 2 &&
             oldtouch.report[1].x / 2 > 336 + 144 - 52 && oldtouch.report[1].y / 2 > 16 + 256 - 29 + 110 &&
             oldtouch.report[1].x / 2 < 336 + 144 + 52 && oldtouch.report[1].y / 2 < 16 + 256 + 29 + 110);
        if (blink == 1)
        {
            gSoloud.play(hit);
